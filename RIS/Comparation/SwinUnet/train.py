...
from config.train_config import train_config as config
if __name__ == '__main__':
    ...
    #dataset = Dataset()
    #dataset.load_data()
    dataset = MyDataLoader(istrain,ispretrain,cfg)
    val_dataset = MyDataLoader(False,True,cfg)
    ...
    model = Pix2PixModel(istrain,ispretrain,cfg)
    ...
    model.set_continue_train(cfg.continue_train)
    ...
    total_iters = 0
    epoch_count = cfg.begin_epoch
    n_epochs_decay = cfg.end_epoch
    n_epochs = cfg.delay_epoch
    is_save_nii = True
    train_save_img = save_img(istrain, ispretrain, read_dataset.get_slice_scope(), n_epochs_decay + n_epochs, cfg)
    val_save_img = save_img(False, True, read_dataset.get_slice_scope(), n_epochs_decay + n_epochs,cfg)
    for epoch in range(epoch_count,n_epochs_decay + n_epochs + 1):
        ...
        print("val:\n")
        for i, test_data in enumerate(val_dataset):
